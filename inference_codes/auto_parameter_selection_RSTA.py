...
def parameter_selection():
  ...
  filenames=['toy10','toy50','emotions','yeast','scene','enron']
  n=0
  for filename in filenames:
    for graph_type in ['tree']:
      for l_norm in ['2']:
        in_i=0
        for in_c in ['100','50','20','10','5','1','0.5','0.25','0.1','0.01']:
          in_i+=1
          for kth_fold in ['1']:
            node=cluster[n%len(cluster)]
            n+=1
            p=multiprocessing.Process(target=singleRSTA, args=(filename,graph_type,'1',node,'1',l_norm,"%d" % in_i,in_c,))
            jobs.append(p)
            p.start()
            time.sleep(10) # fold
            pass
