...
%
%
function [rtn, ts_err] = RSTAprofile(paramsIn, dataIn)
    ...
    Ypred_rank_statistics = zeros(1,4);
...
function profile_update
    ...
    if params.profiling
        ...
        print_message(...
            sprintf('tm: %d 1_er_tr: %d (%3.2f) er_tr: %d (%3.2f) 1_er_ts: %d (%3.2f) er_ts: %d (%3.2f) Ypred* %.2f %.2f %.2f %.2f Ypred* %.2f %.2f %.2f %.2f Mg %.2f%% %.2f %.2f',...
            ...
            Ypred_rank_statistics(1)*100,...
            Ypred_rank_statistics(2),...
            Ypred_rank_statistics(3),...
            Ypred_rank_statistics(4),...
            Ypred_rank_statistics(5)*100,...
            Ypred_rank_statistics(6),...
            Ypred_rank_statistics(7),...
            Ypred_rank_statistics(8),...
            Ypred_rank_statistics(9)*100,...
            Ypred_rank_statistics(10),...
            Ypred_rank_statistics(11)),...
...
function profile_update_tr
    ...
    if params.profiling
        ...
        %Yipos_list
        print_message(...
            sprintf('tm: %d iter: %d 1_er_tr: %d (%3.2f) er_tr: %d (%3.2f) K: %d Y*pos: %3.2f%% %.2f %.2f %d Yipos: %3.2f%% %.2f %.2f %.2f K: %.2f %.2f %d Mg: %.2f%% %.3f %.3f obj: %.2f gap: %.2f%% Update %.2f%% %.2f%% gap: %.2f%% Ypred* %.2f%% %.2f %.2f %.2f Ypred_i %.2f%% %.2f %.2f %.2f Mg %.2f%% %.2f %.2f',...
            round(tm-profile.start_time),...
            opt_round,...
            profile.n_err,...
            profile.p_err*100,...
...
function [Ypred,YpredVal] = compute_error(Y,Kx) 
    ...
    Y_kappa = zeros(size(Y,1)*T_size,size(Y,2)*kappa);
    Y_kappa_val = zeros(size(Y,1)*T_size,kappa);
    Ypred_rank = zeros(size(Y,1),1);
    Ytrue_rank = zeros(size(Y,1),1);
...
%                     Y_kappa_val((i:size(Y_kappa,1)/T_size:size(Y_kappa_val,1)),:),[]);
                ...
                [~,YpredVal(i,:),~,Ytrue_rank(i)] = ...
                    find_worst_violator_new(...
                    Y_kappa((i:size(Y_kappa,1)/T_size:size(Y_kappa,1)),:),...
                    Y_kappa_val((i:size(Y_kappa,1)/T_size:size(Y_kappa_val,1)),:),...
                    (Y(i,:)+1)/2,IN_E,IN_gradient);
                [Ypred(i,:),~,Ypred_rank(i),~] = ...
                    find_worst_violator_new(...
                    Y_kappa((i:size(Y_kappa,1)/T_size:size(Y_kappa,1)),:),...
                    Y_kappa_val((i:size(Y_kappa,1)/T_size:size(Y_kappa_val,1)),:),...
                    [],IN_E,IN_gradient);
        ...
        %YpredVal'
        %[Ypred_rank,Ytrue_rank]'
        Ypred_rank_statistics = [...
            sum(Ypred_rank<=kappa & Ypred_rank>0)/size(Y,1),...
            mean(Ypred_rank(Ypred_rank<=kappa & Ypred_rank>0)),...
            std(Ypred_rank(Ypred_rank<=kappa & Ypred_rank>0)),...
            max(Ypred_rank),...
            sum(Ytrue_rank<=kappa & Ytrue_rank>0)/size(Y,1),...
            mean(Ytrue_rank(Ytrue_rank<=kappa & Ytrue_rank>0)),...
            std(Ytrue_rank(Ytrue_rank<=kappa & Ytrue_rank>0)),...
            max(Ytrue_rank),...
            sum(YpredVal>=0)/size(Y,1),...
            mean(YpredVal),...
            std(YpredVal),...
            ];
