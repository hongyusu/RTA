...
def singleRSTA(filename,graph_type,t,node):
  try:
    with open("../outputs/%s_%s_%s_RSTA.log" % (filename,graph_type,t)): pass
    logging.info('\t--< (node)%s,(f)%s,(type)%s,(t)%s' %( node,filename,graph_type,t))
  except:
    logging.info('\t--> (node)%s,(f)%s,(type)%s,(t)%s' %( node,filename,graph_type,t))
    os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd /home/group/urenzyme/workspace/colt2014/experiments/inference_codes/; nohup matlab -nodisplay -nosplash -nodesktop -r "profile_RSTA '%s' '%s' '%s' " > /var/tmp/tmp_%s_%s_%s_RSTA' """ % (node,filename,graph_type,t,filename,graph_type,t) )
    logging.info('\t--| (node)%s,(f)%s,(type)%s,(t)%s' %( node,filename,graph_type,t))
    time.sleep(5)
  pass
